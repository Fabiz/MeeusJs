...
A.MoonIllum = {
	...
	phaseAngleEq: function (eqcoordm, deltam, eqcoords, deltas)  {
		return A.MoonIllum._phaseangle(deltam, deltas, A.MoonIllum._clngEq(eqcoordm, eqcoords));
	},
	...
	phaseAngleEq2: function (eqcoordm, eqcoords) {
		return Math.acos(-A.MoonIllum._clngEq(eqcoordm, eqcoords));
	},

	/**
	 * illumFraction returns the illuminated fraction of the moon. 
	 * 
	 ...
	 * @param {A.EqCoord} eqcoordm - geocentric right ascension and declination of the Moon
	 * @param {A.EqCoord} eqcoords - geocentric right ascension and declination of the Sun
	 * @return {number} position angle in radians
	 */
	 ...
	 */
	_clngEq: function (eqcoordm, eqcoords) {
		var sdecm = Math.sin(eqcoordm.dec);
		var cdecm = Math.cos(eqcoordm.dec);
		var sdecs = Math.sin(eqcoords.dec);
		var cdecs = Math.cos(eqcoords.dec);
	 ...
	 */
	_phaseangle: function (deltam, deltas, clng) {
		var slng = Math.sin(Math.acos(clng));
		var i = Math.atan(deltas * slng / (deltam - deltas*clng));
		if (i < 0) {
			i += Math.PI;
		}
		return i;
