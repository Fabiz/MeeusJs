...
A.Solar = {
	...
	approxTimes: function (jdo, eclCoord) {
		...
		var aet = A.Solar.apparentTopocentric(jdo, eclCoord);
	...
	times: function (jdo, eclCoord) {
		jdo = jdo.startOfDay(); // make sure jd is at midnight (ends with .5)
		var aet1 = A.Solar.apparentTopocentric(new A.JulianDay(jdo.jd-1, jdo.deltaT), eclCoord);
		var aet2 = A.Solar.apparentTopocentric(jdo, eclCoord);
		var aet3 = A.Solar.apparentTopocentric(new A.JulianDay(jdo.jd+1, jdo.deltaT), eclCoord);
			
		var h0 = A.Rise.stdh0Solar; // stdh0Stellar
		var Th0 = A.Sidereal.apparent0UT(jdo);
		
		return A.Rise.times(eclCoord, jdo.deltaT, h0, Th0, [aet1, aet2, aet3]);
	},
