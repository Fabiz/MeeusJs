...
A.JulianDay.jdToDate=function(a){var b=A.JulianDay.jdToCalendar(a);a=A.Math.modF(a+.5)[1];a=Math.round(86400*a);return new Date(Date.UTC(b.y,b.m-1,b.d,Math.floor(a/3600)%24,Math.floor(a/60)%60,Math.floor(a%60)))};
...
[134.9633964*b,477198.8675055*b,.0087414*b,b/69699,-b/14712E3]),2*Math.PI),g=A.Math.pMod(A.Math.horner(c,[93.272095*b,483202.0175233*b,-.0036539*b,-b/3526E3,b/86331E4]),2*Math.PI),h=119.75*b+131.849*b*c,m=53.09*b+479264.29*b*c,k=313.45*b+481266.484*b*c,c=A.Math.horner(c,[1,-.002516,-7.4E-6]),p=c*c,m=3958*Math.sin(h)+1962*Math.sin(a-g)+318*Math.sin(m),n=0,h=-2235*Math.sin(a)+382*Math.sin(k)+175*Math.sin(h-g)+175*Math.sin(h+g)+127*Math.sin(a-f)-115*Math.sin(a+f),l,k=0;k<A.Moon.ta.length;k++){l=A.Moon.ta[k];
var r=d*l[0]+e*l[1]+f*l[2]+g*l[3],q=Math.sin(r),r=Math.cos(r);switch(l[1]){case 0:m+=l[4]*q;n+=l[5]*r;break;case 1:case -1:m+=l[4]*q*c;n+=l[5]*r*c;break;case 2:case -2:m+=l[4]*q*p;n+=l[5]*r*p;break;default:throw"error";}}for(k=0;k<A.Moon.tb.length;k++)switch(l=A.Moon.tb[k],q=Math.sin(d*l[0]+e*l[1]+f*l[2]+g*l[3]),l[1]){case 0:h+=l[4]*q;break;case 1:case -1:h+=l[4]*q*c;break;case 2:case -2:h+=l[4]*q*p;break;default:throw"error";}return{lng:A.Math.pMod(a,2*Math.PI)+1E-6*m*b,lat:1E-6*h*b,delta:385000.56+
...
Math.cos(b.ra-a.ra)}};A.Nutation={nutation:function(a){a=a.jdeJ2000Century();for(var b=A.Math.horner(a,[297.85036,445267.11148,-.0019142,1/189474])*Math.PI/180,c=A.Math.horner(a,[357.52772,35999.05034,-1.603E-4,-1/3E5])*Math.PI/180,d=A.Math.horner(a,[134.96298,477198.867398,.0086972,1/5620])*Math.PI/180,e=A.Math.horner(a,[93.27191,483202.017538,-.0036825,1/327270])*Math.PI/180,f=A.Math.horner(a,[125.04452,-1934.136261,.0020708,1/45E4])*Math.PI/180,g=0,h=0,m=A.Nutation.table22A.length-1;0<=m;m--)var k=A.Nutation.table22A[m],
p=k[0]*b+k[1]*c+k[2]*d+k[3]*e+k[4]*f,n=Math.cos(p),g=g+Math.sin(p)*(k[5]+k[6]*a),h=h+n*(k[7]+k[8]*a);return{deltalng:Math.PI/180*(1E-4/3600)*g,deltaobliquity:Math.PI/180*(1E-4/3600)*h}},nutationInRA:function(a){var b=A.Nutation.meanObliquityLaskar(a);a=A.Nutation.nutation(a);return a.deltalng*Math.cos(b+a.deltaobliquity)},trueObliquity:function(a){var b=A.Nutation.meanObliquityLaskar(a);a=A.Nutation.nutation(a);return b+a.deltaobliquity},meanObliquity:function(a){return A.Math.horner(a.jdeJ2000Century(),
...
e,c,d)},times:function(a,b){var c=a.startOfDay(),d=A.Solar.apparentTopocentric(new A.JulianDay(c.jd-1,c.deltaT),b),e=A.Solar.apparentTopocentric(c,b),f=A.Solar.apparentTopocentric(new A.JulianDay(c.jd+1,c.deltaT),b),g=A.Rise.stdh0Solar,h=A.Sidereal.apparent0UT(c);return A.Rise.times(b,c.deltaT,g,h,[d,e,f])},meanAnomaly:function(a){return A.Math.horner(a,[357.52911,35999.05029,-1.537E-4])*Math.PI/180},trueLongitude:function(a){var b=A.Math.horner(a,[280.46646,36000.76983,3.032E-4])*Math.PI/180,c=A.Solar.meanAnomaly(a);
...
A.JulianCentury,e=35999.373*Math.PI/180*d-2.47*Math.PI/180,e=1+.0334*Math.cos(e)+7E-4*Math.cos(2*e),f=0,g=this.terms.length-1;0<=g;g--)var h=this.terms[g],f=f+h[0]*Math.cos((h[1]+h[2]*d)*Math.PI/180);return c+1E-5*f/e},mc0:[1721139.29189,365242.1374,.06134,.00111,-7.1E-4],jc0:[1721233.25401,365241.72562,-.05232,.00907,2.5E-4],sc0:[1721325.70455,365242.49558,-.11677,-.00297,7.4E-4],dc0:[1721414.39987,365242.88257,-.00769,-.00933,-6E-5],mc2:[2451623.80984,365242.37404,.05169,-.00411,-5.7E-4],jc2:[2451716.56767,
